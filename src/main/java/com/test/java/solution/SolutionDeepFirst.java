...
public class SolutionDeepFirst {
    ...
    public List<List<Integer>> pacificAtlantic(int[][] matrix) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) {
                ...
                if (isPacific(i, j, i, j, matrix, visited) && isAtlantic(i, j, i, j, matrix, visited)) {
    ...
    boolean isPacific(int lastx, int lasty, int x, int y, int[][] matrix, boolean[][] visited) {
        //判断边界
        ...
        //判断大小
        if (matrix[x][y] > matrix[lastx][lasty]) {
            return false;
        }
        if (x == 0 || y == 0) {
            return true;
        ...
        visited[x][y] = true;
        boolean flag = isPacific(x, y, x - 1, y, matrix, visited) || isPacific(x, y, x, y - 1, matrix, visited) || isPacific(x, y, x + 1, y, matrix, visited) || isPacific(x, y, x, y + 1, matrix, visited);
        visited[x][y] = false;
        return flag;
    }

    boolean isAtlantic(int lastx, int lasty, int x, int y, int[][] matrix, boolean[][] visited) {
        //判断边界
        if (x < 0 || x >= matrix.length || y < 0 || y >= matrix[0].length) {
            return false;
        }
        ...
        visited[x][y] = true;
        boolean flag = isAtlantic(x, y, x - 1, y, matrix, visited) || isAtlantic(x, y, x, y - 1, matrix, visited) || isAtlantic(x, y, x + 1, y, matrix, visited) || isAtlantic(x, y, x, y + 1, matrix, visited);
        visited[x][y] = false;
        return flag;
    }
    ...
    public List<List<Integer>> pacificAtlantic2(int[][] matrix) {
        List<List<Integer>> resultList = new ArrayList<>();
        // 0  未走过， 1 不能到达   2 能到达，
        int[][] pacificArray = new int[matrix.length][matrix[0].length];
        int[][] atlanticArray = new int[matrix.length][matrix[0].length];
        boolean[][] visited = new boolean[matrix.length][matrix[0].length];
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) {
                boolean isPacific = isPacific(i, j, i, j, matrix, pacificArray, visited);
                boolean isAtlantic = isAtlantic(i, j, i, j, matrix, atlanticArray, visited);
    ...
    boolean isPacific(int lastx, int lasty, int x, int y, int[][] matrix, int[][] pacificArray, boolean[][] visited) {
        //判断边界
        if (x < 0 || x >= matrix.length || y < 0 || y >= matrix[0].length) {
            return false;
        }
        ...
        //已经走过
        if (pacificArray[x][y] > 0) {
            return pacificArray[x][y] == 2;
        }
        //如果已经到达边界
        if (x == 0 || y == 0) {
            pacificArray[x][y] = 2;
        ...
        visited[x][y] = true;
        //未到达向上下左右走
        boolean flag = isPacific(x, y, x - 1, y, matrix, pacificArray, visited) || isPacific(x, y, x, y - 1, matrix, pacificArray, visited) || isPacific(x, y, x + 1, y, matrix, pacificArray, visited) || isPacific(x, y, x, y + 1, matrix, pacificArray, visited);
        pacificArray[x][y] = 1;
        visited[x][y] = false;
        return flag;
    }

    boolean isAtlantic(int lastx, int lasty, int x, int y, int[][] matrix, int[][] atlanticArray, boolean[][] visited) {
        //判断边界
        if (x < 0 || x >= matrix.length || y < 0 || y >= matrix[0].length) {
            return false;
        }
        ...
        //已经走过
        if (atlanticArray[x][y] > 0) {
            return atlanticArray[x][y] == 2;
        }
        if (x == matrix.length - 1 || y == matrix[0].length - 1) {
            atlanticArray[x][y] = 2;
            return true;
        ...
        visited[x][y] = true;
        boolean flag = isAtlantic(x, y, x - 1, y, matrix, atlanticArray, visited) || isAtlantic(x, y, x, y - 1, matrix, atlanticArray, visited) || isAtlantic(x, y, x + 1, y, matrix, atlanticArray, visited) || isAtlantic(x, y, x, y + 1, matrix, atlanticArray, visited);
        atlanticArray[x][y] = flag ? 2 : 1;
        visited[x][y] = false;
        return flag;
    }
}
